...
class Model(object):
    ...
    def inference(x, drop_rate, regularization=None):
        net = tf.layers.conv2d(x, filters=48, kernel_size=[5, 5], padding='same', activation=tf.nn.relu)
        ...
        net = tf.layers.conv2d(net, filters=160, kernel_size=[5, 5], padding='same', activation=tf.nn.relu)
        net = tf.layers.max_pooling2d(net, pool_size=[2, 2], strides=2, padding='same')
